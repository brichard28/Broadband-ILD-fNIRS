...
% Updated: 12-Jul-2016 (Robert Baumgartner)
% Updated: 06-Nov-2020 (Jasmine Kwasa) to work for 2 sample comparisons
% (uses ttest2)

if nargin < 4
    thresh = 0.95; % t-distribution threshold
    alpha = 0.05;  % significance level
    tail = 'left'; % t-test tail -- i.e. A < B
elseif nargin < 5
    alpha = 0.05;
    tail = 'left';
elseif nargin < 6
...
nA = size(a,1); %20
nB = size(b,1); %25
df = min([nA,nB])-1;  % degrees of freedom
...
[Xobs.h,Xobs.p,Xobs.ci,Xobs.stats] = ttest2(a,b,'Tail',tail); %left means: h1 = a < b, h0 = a >= b
% Q FROM 1/13:DOES TTEST2 INTRODUCE BIAS???
[ObsClstrStat,kClstr] = findClusters(Xobs.stats.tstat,thresh,df,tail); % kClstr = indices of clusters
...
[f,x] = ecdf(permStat); %f = cum density (0 to 1), x = random variable  (T val i think)

% Determine alpha significance value, tValue, and test decision variable
switch tail
    case 'right'
        tVal = x(find(f>=(1-alpha),1,'first'));
        if any(ObsClstrStat>=tVal)
            for n = find(ObsClstrStat>=tVal)
                h(kClstr{n}) = 1;
            end
        end
    case 'left'
        tVal = x(find(f<=alpha,1,'last')); %p value associated with our alpha 
        if any(ObsClstrStat<=tVal)
            for n = find(ObsClstrStat<=tVal)
                h(kClstr{n}) = 1; %label the full cluster significant
            end
        end
end
