...
%    '028MC','028XX','029MX','030MX','031MC',...
       

    %'030XX', % MATLAB couldn't save the file for this one
...
%power_all_subjects = zeros(length(subject_tags),240,64,195,160); % subjects x trials x freqs, time

eeglab % need it open for newtimef
fs = 256;
%addpath('C:\Users\benri\Documents\PhD Year 1\Jasmine EEG Data\BNR Final DATA Files')
include_incorrect = 0; % logical(input('Include incorrect trials? (1 or 0) '));
channel_selection = 'All';% string(input('Please enter the channels you would like to analyze (form: LPO for Left Parietoocciptal, APO for all parietoocciptal): ','s'));
...
subtract_evoked = 1; %logical(input('Subtract out mean evoked data? (1 or 0)')); 
...
for isubject = 1:length(subject_tags)
    ...
    filename = append(string(['AUD_ASA',num_tag,'_',type_tag,'_1-50_DATA.mat']));
    load(filename);
    all_trials = 1:240;
    ...
    erp_size = size(ERP.erp);
    trials_to_run = 1:size(ERP.erp,3);
    disp('trials to run size is');
    disp(size(ERP.erp,3));
    ...
    for ichannel = channels_to_run
        parfor itrial = trials_to_run
            ...
            [P,f,~] = spectrogram(this_EEG,nseg,overlap,0:1:50,fs);
            t = linspace(epoch_time_limits(1)*1000,epoch_time_limits(2)*1000,size(P,2));
            P = 20*log10(abs(P));
            ...
            ersp(itrial,ichannel,:,:) = (P - squeeze(resting_ersp_mean(itrial,ichannel,:)))./squeeze(resting_ersp_std(itrial,ichannel,:));
    ...
    %power_all_subjects(isubject,:,:,:,:) = ersp; % subjects x trial types X channels X freqs X times

    power = ersp;
    filename = append(string(subject_tags(isubject)),'_spectrogram_bytrial a la Shim.mat');
    num_trials_used = length(trials_to_run);
    times = squeeze(mean(times,[1,2]));
    freqs = squeeze(mean(freqs,[1,2]));
    save(filename,'power','num_trials_used','freqs','times','-v7.3');
